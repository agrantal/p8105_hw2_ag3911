...
filter(year == 17) %>% summarize(sum(total_precip))`. The median of number of sports balls in a dumpster in 2016 was `r median(trash_data$sports_balls)`. 
...
#should remove the denom and num vars: 
remove(brfss$`ex_or_vg_denom`, brfss$`ex_or_vg_num`)
```
...
ggplot(brfss_NYC_Queens, aes(x = year, y = ex_prop)) + geom_point()
```
