...
The edited trash dataset has dimensions `r dim(trash_data)`. The joined and edited precipitation dataset has dimensions `r  dim(precip_joined)`. The total precipitation in 2017 was `r sum(precip_joined$total_precip_17)`. The median of number of sports balls in a dumpster in 2016 was `r median(trash_data$sports_balls)`. 
...
mutate(brfss, "Very Good" = as.numeric("Very Good")) 
mutate(brfss, Good = as.numeric(Good))
mutate(brfss, Fair = as.numeric(Fair))
mutate(brfss, Poor = as.numeric(Poor))
mutate(brfss, Excellent = as.numeric(Excellent))

# how to make proportional var ??
# how to rearrange order to list (left to right) excellent, very good, good, fair, poor ??
```
